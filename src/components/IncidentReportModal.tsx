...
export function IncidentReportModal({ log, onClose }: IncidentReportModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#F3F1ED] dark:bg-[#101010]/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl glass-panel shadow-2xl shadow-black/50 animate-in zoom-in-95 duration-200">
        <header className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 bg-white dark:bg-[#171427]/80 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">AI Incident Report</h2>
          ...
          <button
            onClick={onClose}
            className="rounded-full p-2 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:bg-white/10 hover:text-slate-900 dark:text-white focus:outline-none"
          >
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        <div className="p-6 glass-surface">
          <div className="mb-6 rounded-xl border border-red-500/20 bg-red-500/10 p-4">
            ...
            <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">
              AI recommended <strong className="text-slate-900 dark:text-white">{log.actionTaken}</strong> for <strong className="text-slate-900 dark:text-white">{log.assetId}</strong>, but Human Admin overrode the decision to <strong className="text-slate-900 dark:text-white">{log.humanDecision}</strong>.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-[#F3F1ED] dark:bg-[#101010]/50 p-4">
        ...
        <footer className="border-t border-slate-200 dark:border-white/10 bg-white dark:bg-[#171427]/80 px-6 py-4 text-right">
          <button
            onClick={onClose}
