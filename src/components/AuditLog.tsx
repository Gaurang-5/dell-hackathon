...
export function AuditLog({ logs, globalSearchQuery = '' }: AuditLogProps) {
  ...
  const filteredAndSortedLogs = useMemo(() => {
    return logs
      .filter(log => {
        ...
        const matchesSearch = log.assetId.toLowerCase().includes(query.toLowerCase()) || 
                              log.actionTaken.toLowerCase().includes(query.toLowerCase()) ||
                              log.reasoningSummary.toLowerCase().includes(query.toLowerCase())
  ...
  }, [logs, searchQuery, globalSearchQuery, filterDecision, sortBy])
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-300">
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-2">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Audit Log</h1>
        ...
        <div className="glass-surface overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-white dark:bg-[#171427]/50 text-slate-700 dark:text-slate-300 font-medium border-b border-slate-200 dark:border-white/10">
                <tr>
              ...
              <tbody className="divide-y divide-white/5">
                ...
                return (
                  <tr key={log.id} className="hover:bg-white/50 dark:hover:bg-white/5 hover:backdrop-blur-sm transition-all cursor-pointer">
                    ...
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{log.actionTaken}</td>
                      ...
                      <div className="flex flex-col gap-2 items-start">
                        ...
                        {(log.humanDecision === 'Overridden' || log.humanDecision === 'Escalated') && (
                          <button
                            ...
                            className="text-xs font-semibold text-[#EDE9FE] hover:text-slate-900 dark:text-white hover:underline"
                          >
